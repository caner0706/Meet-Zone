...
import { STAJ_MEETING_REPORTS } from '../../features/staj/meetingReports'
...
/** Rapor metnini tek bir string olarak toplar (seslendirme için) */
...
export default function StajReport() {
  const [selectedId, setSelectedId] = useState(null)
  const [isSpeaking, setIsSpeaking] = useState(false)
  const speechSynthRef = useRef(null)
  const selected = useMemo(
    () => STAJ_MEETING_REPORTS.find((r) => r.id === selectedId),
    [selectedId]
  )

  // ——— Liste görünümü (öğretmen ders raporları ile aynı kart yapısı) ———
  if (!selected) {
    return (
      <>
        <header className="page-header report-page-header animate-enter">
          ...
          <p className="page-subtitle">
            Staj sürecinde gerçekleştirdiğiniz toplantıların raporları. Raporu görmek için toplantıya tıklayın.
          </p>
        </header>
        <div className="report-lesson-list animate-on-scroll visible" data-animate>
          {STAJ_MEETING_REPORTS.map((report, index) => (
            <button
              key={report.id}
              type="button"
  ...
  // ——— Detay: öğretmen ders raporu ile aynı şablon (report-document, report-cover, report-section ...) ———
        ...
        <div className="report-document animate-on-scroll visible" data-animate>
          <div className="report-cover">
            ...
            <p className="report-cover-date">
              {formatReportDateShort(selected.date)} · {selected.durationMin} dk · Staj
            </p>
          </div>
          ...
          {/* Alınan kararlar (staj raporuna özel) */}
          ...
          {/* Aksiyon maddeleri (staj raporuna özel) */}
          ...
          {/* Ders transkripti (kim ne konuştu) */}
          {transcript.length > 0 && (
            <section className="report-section">
