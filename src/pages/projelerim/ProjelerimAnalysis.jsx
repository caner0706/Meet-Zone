...
import { PROJE_MEETING_ANALYSES } from '../../features/projelerim/projelerimAnalysisData'
...
export default function ProjelerimAnalysis() {
  const [selectedId, setSelectedId] = useState(null)
  const selected = useMemo(
    () => PROJE_MEETING_ANALYSES.find((a) => a.id === selectedId),
    [selectedId]
  )
      ...
      <>
        <header className="page-header meeting-analysis-header animate-enter">
          ...
          <p className="page-subtitle">
            TÜBİTAK, Turkcell ve Teknofest proje toplantılarında DAISEE modeli ile odak, stress, tereddüt ve sıkılmışlık dağılımı. Bir toplantı seçin.
          </p>
        </header>
        <div className="meeting-analysis-list animate-on-scroll visible" data-animate>
          {PROJE_MEETING_ANALYSES.map((m, idx) => (
            <button
              key={m.id}
              type="button"
          ...
          <div className="live-analysis-chart-inner live-analysis-chart-inner--line">
            <ResponsiveContainer width="100%" height={300}>
              ...
              >
                ...
                <Line type="monotone" dataKey="sikilmis" name="İlgi kaybı" stroke="#8B5CF6" strokeWidth={2} dot={false} isAnimationActive animationDuration={500} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
